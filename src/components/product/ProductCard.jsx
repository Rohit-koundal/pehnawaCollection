...
export function ProductVisual({ product, compact = false }) {
  ...
  return (
    <div className={`relative overflow-hidden bg-[#F5F5F6] ${compact ? 'h-44' : 'h-64'}`}>
      <div className="absolute inset-x-5 bottom-0 h-[86%] rounded-t-[90px]" style={{ background: `linear-gradient(145deg, ${color}, #ffd6c7)` }} />
...
export default function ProductCard({ product, navigate }) {
  ...
  return (
    <article className="overflow-hidden rounded border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="space-y-2 p-3">
        ...
        <button onClick={() => cart.addToCart(product)} className="h-10 w-full rounded bg-rose text-xs font-black text-white">
          Add to Bag
        </button>
      </div>
    </article>
