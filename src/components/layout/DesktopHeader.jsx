...
export default function DesktopHeader({ navigate }) {
  ...
  return (
    <header className="sticky top-0 z-50 hidden bg-white/95 shadow-sm backdrop-blur md:block">
      <div className="bg-wine px-4 py-2 text-center text-xs font-bold uppercase tracking-[0.22em] text-white">
        Pehnawa Collection Dungar | Free Shipping Above Rs. 999 | Fresh Styles Live
      </div>
      <div className="container-page flex h-20 items-center gap-6">
        <button onClick={() => navigate('/')} className="shrink-0" aria-label="Pehnawa Collection Dungar home">
          <img src={logo} alt="Pehnawa Collection Dungar" className="h-14 w-auto" />
        </button>
        ...
        <button onClick={() => navigate('/search')} className="flex h-11 min-w-[280px] items-center gap-3 rounded bg-[#F5F5F6] px-4 text-sm font-semibold text-slate-500">
          <Icon name="search" className="h-4 w-4" />
          Search Dungar fits, sarees, suits...
        </button>
