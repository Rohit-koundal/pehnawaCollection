...
const demoUsers = {
  'admin@pehnawacollection.com': { name: 'Admin', email: 'admin@pehnawacollection.com', role: 'admin' },
...
export function AuthProvider({ children, navigate }) {
  const [user, setUser] = useState(() => {
    const stored = localStorage.getItem('pehnawa_user');
    return stored ? JSON.parse(stored) : null;
  });
  const [toast, setToast] = useState('');

  const login = useCallback(({ email }) => {
    const account = demoUsers[email] || { name: 'Pehnawa Customer', email, role: 'customer' };
    localStorage.setItem('pehnawa_user', JSON.stringify(account));
    localStorage.setItem('pehnawa_token', 'demo-jwt-token');
    setUser(account);
  ...
  const logout = useCallback(() => {
    localStorage.removeItem('pehnawa_user');
    localStorage.removeItem('pehnawa_token');
    setUser(null);
    navigate('/');
  }, [navigate]);
